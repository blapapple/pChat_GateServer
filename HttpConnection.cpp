...
/*
 * 处理Http请求
 * 这里使用了boost::beast库来解析HTTP请求，并根据请求的类型进行处理。
 * 如果请求是GET方法，则调用LogicSystem的HandleGet方法来处理请求。
 * 如果请求的URL没有找到，则返回404错误。
 */
...
/*
 * 检查连接是否超时
 * 这里使用了boost::asio::steady_timer来设置一个定时器，定时器到期后会关闭socket连接
 */
